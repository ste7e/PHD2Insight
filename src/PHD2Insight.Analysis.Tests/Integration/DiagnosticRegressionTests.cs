...
public sealed class DiagnosticRegressionTests {
    ...
    public void RunRegressionTestOnAllLogsInFolder() {
        ...
        ReportFrequencyAndPeriodMetrics();

        var path = Path.Combine(
            AppContext.BaseDirectory,
            "mechanical-period-analysis.csv");

        WriteMechanicalPeriodCsv(path);
    ...
    private static void AccumulateFrequencyAndPeriodMetrics(
        ...
        var raAlgorithm =
            GuideAlgorithmClassifier.Classify(
                session.XGuideAlgorithm);

        var decAlgorithm =
            GuideAlgorithmClassifier.Classify(
                session.YGuideAlgorithm);

        foreach (var result in analysis.OscillationMetrics.RaMechanicalPeriods) {
            if (!double.IsFinite(result.Power)) {
                continue;
            }

            var key =
                $"RA: {raAlgorithm}: {result.Period.Name}";

            if (!mechanicalPeriodPowers.TryGetValue(key, out var powers)) {
                powers = [];
                mechanicalPeriodPowers[key] = powers;
            }

            powers.Add(result.Power);
        }

        foreach (var result in analysis.OscillationMetrics.DecMechanicalPeriods) {
            if (!double.IsFinite(result.Power)) {
                continue;
            }

            var key =
                $"DEC: {decAlgorithm}: {result.Period.Name}";
    ...
    GuidingSession session) {
        ...
        var raAlgorithm =
            GuideAlgorithmClassifier.Classify(session.XGuideAlgorithm);

        var decAlgorithm =
            GuideAlgorithmClassifier.Classify(session.YGuideAlgorithm);

        var raPower = analysis.OscillationMetrics.RaMechanicalPeriods
            .Where(x => double.IsFinite(x.Power))
            .ToDictionary(
                x => x.Period.PeriodSeconds,
                x => x.Power);

        var decPower = analysis.OscillationMetrics.DecMechanicalPeriods
            .Where(x => double.IsFinite(x.Power))
            .ToDictionary(
                x => x.Period.PeriodSeconds,
                x => x.Power);

        mechanicalPeriodCsvRows.Add(
            new MechanicalPeriodCsvRow(
                session.StartTime,
                raAlgorithm.ToString(),
                decAlgorithm.ToString(),
                analysis.Rms.TotalArcSeconds,
                raPower,
                decPower));
    }

    private static void WriteMechanicalPeriodCsv(
    string fileName) {

        var raPeriods = MountPeriodProfiles.Eq6RPro.RaPeriods;
        var decPeriods = MountPeriodProfiles.Eq6RPro.DecPeriods;

        using var writer = new StreamWriter(fileName);

        writer.Write("StartTime,RAAlgorithm,DECAlgorithm,TotalRms");

        foreach (var period in raPeriods) {
            writer.Write(
                $",RA_{period.PeriodSeconds:F2}");
        }

        foreach (var period in decPeriods) {
            writer.Write(
                $",DEC_{period.PeriodSeconds:F2}");
        }

        writer.WriteLine();

        foreach (var row in mechanicalPeriodCsvRows
            .OrderBy(x => x.StartTime)) {

            writer.Write(
                $"{row.StartTime:yyyy-MM-dd HH:mm:ss}," +
                $"{row.RaAlgorithm}," +
                $"{row.DecAlgorithm}," +
                $"{row.TotalRms:F4}");

            foreach (var period in raPeriods) {
                WritePower(writer, row.RaPower, period.PeriodSeconds);
            }

            foreach (var period in decPeriods) {
                WritePower(writer, row.DecPower, period.PeriodSeconds);
            }
    ...
    private static void WritePower(
        StreamWriter writer,
        IReadOnlyDictionary<double, double> powers,
        double periodSeconds) {

        writer.Write(',');

        if (powers.TryGetValue(periodSeconds, out var power) &&
            double.IsFinite(power)) {
            writer.Write(power.ToString("F6",
...
double.IsFinite(value)
    ...
    private static string FormatPeriod(double? value) {
        return value.HasValue
            ? $"{value.Value:F1}s"
            : "n/a";
    }
    ...
    static void ReportFrequencyAndPeriodMetrics() {
        Console.WriteLine();
        Console.WriteLine("=================================");
        Console.WriteLine("Mechanical Period Power");
        Console.WriteLine("=================================");

        ReportMechanicalPeriodPower(
            "RA",
            GuideAlgorithmType.Hysteresis);
    ...
    string axis,
    GuideAlgorithmType algorithm) {

        var prefix = $"{axis}: {algorithm}: ";

        var entries = mechanicalPeriodPowers
            .Where(x => x.Key.StartsWith(
                prefix,
                StringComparison.Ordinal))
            .OrderByDescending(x => Median(x.Value))
            .ToArray();
        ...
        Console.WriteLine();
        Console.WriteLine(
            $"{axis} Mechanical Period Power — {algorithm}");
        ...
        foreach (var entry in entries) {
            ...
            Console.WriteLine(
                $"  {periodName}: " +
                $"{entry.Value.Count} sessions, " +
                $"median power {Median(entry.Value):F3}");
        }
    }
    ...
    private sealed record MechanicalPeriodCsvRow(
        DateTime StartTime,
        string RaAlgorithm,
        string DecAlgorithm,
        double TotalRms,
        IReadOnlyDictionary<double, double> RaPower,
        IReadOnlyDictionary<double, double> DecPower);
